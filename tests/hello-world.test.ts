...
describe("test contract functions", () => {
  it("returns 'Hello World' from say-hi", () => {
    ...
    expect(sayHiResponse.result).toStrictEqual(Cl.ok(Cl.stringAscii("Hello World")));
  });
  ...
  it("returns the same number from echo-number", () => {
    ...
    expect(echoNumberResponse.result).toStrictEqual(Cl.ok(Cl.int(42)));
  });
  ...
  it("returns ok when check-it is true", () => {
    ...
    expect(checkItResponseTrue.result).toStrictEqual(Cl.ok(Cl.int(1)));
  });
